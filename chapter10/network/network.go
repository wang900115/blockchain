...
var (
	nodeAddress     string                                    // 當前節點
	minerAddress    string                                    // 礦工
	KnownNodes      = []string{"localhost:3000"}              // 已經節點
	blocksInTransit = [][]byte{}                              // 正在傳輸的區塊
	memoryPool      = make(map[string]blockchain.Transaction) // 暫存還沒被打包進區塊的交易
)
...
func SendInv(addr, kind string, item [][]byte) {
