...
func NewTransaction(from, to string, amount int, UTXO *UTXOSet) *Transaction {
	...
	wallets, err := wallet.CreateWallets(from)
	Handle(err)

	w := wallets.GetWallet(from)
	pubKeyHash := wallet.PublicKeyHash(w.PublicKey)

	acc, validOutpus := UTXO.FindSpendableOutputs(pubKeyHash, amount)
...
func (tx *Transaction) String() string {
	...
	for i, output := range tx.Outputs {
		lines = append(lines, fmt.Sprint(" Output %d:", i))
		lines = append(lines, fmt.Sprint(" 	Value: %d", output.Value))
		lines = append(lines, fmt.Sprint(" 	Script: %d", output.PubKeyHash))
	}
